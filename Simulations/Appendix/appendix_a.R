...
system.time({
  matrix <- foreach(j=1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      tau_bcf_iv00[j*2-1, which(seq==i)]  <- summary(ivreg(y ~ w + x3 + x4 | z + x3 + x4 , data = subset00))$coef[1,1]
      tau_bcf_iv11[j*2, which(seq==i)]  <- summary(ivreg(y ~ w + x3 + x4 | z + x3 + x4 , data = subset11))$coef[1,1]
      se_bcf_iv00[j*2-1, which(seq==i)]  <- summary(ivreg(y ~ w + x3 + x4 | z + x3 + x4 , data = subset00))$coef[1,2]
      se_bcf_iv11[j*2, which(seq==i)]  <- summary(ivreg(y ~ w + x3 + x4 | z + x3 + x4 , data = subset11))$coef[1,2]
...
colnames(results_sims) <- c("correct_rules hct-iv",
                            ...
                            "Bias BCF-IV 11",
                            "Coverage BCF-IV 11")
write.csv(results_sims, file = "sim_confounding_1000.csv")