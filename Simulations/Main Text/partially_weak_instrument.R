...
for(i in seq){
  mse_tau_grf_00[which(seq==i)] <- mean( (abs(tau_grf_00[,which(seq==i)]) - 1)^2 , na.rm = TRUE )
  mse_tau_bcf_iv_00[which(seq==i)] <- mean( (abs(tau_bcf_iv_00[,which(seq==i)]) - 1)^2 , na.rm = TRUE )
  bias_tau_grf_00[which(seq==i)] <- mean( (abs(tau_grf_00[,which(seq==i)]) - 1) , na.rm = TRUE )
  bias_tau_bcf_iv_00[which(seq==i)] <- mean( (abs(tau_bcf_iv_00[,which(seq==i)]) - 1) , na.rm = TRUE )
}
...
for(i in seq){
  mse_tau_grf_11[which(seq==i)] <- mean( (abs(tau_grf_11[,which(seq==i)]) - 1)^2 , na.rm = TRUE )
  mse_tau_bcf_iv_11[which(seq==i)] <- mean( (abs(tau_bcf_iv_11[,which(seq==i)]) - 1)^2 , na.rm = TRUE )
  bias_tau_grf_11[which(seq==i)] <- mean( (abs(tau_grf_11[,which(seq==i)]) - 1) , na.rm = TRUE )
  bias_tau_bcf_iv_11[which(seq==i)] <- mean( (abs(tau_bcf_iv_11[,which(seq==i)]) - 1) , na.rm = TRUE )
}
...
for(i in seq){
  for(j in 1:nrow(tau_grf_00)) {
    if (is.na(tau_grf_00[j,which(seq==i)])==FALSE) {
      coverage_tau_grf_00[j,which(seq==i)] <- between(1, abs(tau_grf_00[j,which(seq==i)]) - 1.96*se_grf_00[j,which(seq==i)],
...
for(i in seq){
  for(j in 1:nrow(tau_bcf_iv_00)) {
    if (is.na(tau_bcf_iv_00[j,which(seq==i)])==FALSE) {
      coverage_tau_bcf_iv_00[j,which(seq==i)] <- between(1, abs(tau_bcf_iv_00[j,which(seq==i)]) - 1.96*se_bcf_iv_00[j,which(seq==i)],
...
for(i in seq){
  for(j in 1:nrow(tau_grf_11)) {
    if (is.na(tau_grf_11[j,which(seq==i)])==FALSE) {
      coverage_tau_grf_11[j,which(seq==i)] <- between(1, abs(tau_grf_11[j,which(seq==i)]) - 1.96*se_grf_11[j,which(seq==i)],
...
for(i in seq){
  for(j in 1:nrow(tau_bcf_iv_11)) {
    if (is.na(tau_bcf_iv_11[j,which(seq==i)])==FALSE) {
      coverage_tau_bcf_iv_11[j,which(seq==i)] <- between(1, abs(tau_bcf_iv_11[j,which(seq==i)]) - 1.96*se_bcf_iv_11[j,which(seq==i)],
...
colnames(results_sims) <- c("correct_rules hct-iv",
                            ...
                            "Bias BCF-IV 11",
                            "Coverage BCF-IV 11")
write.csv(results_sims, file = "sim_partially_weak_scenario1_1000.csv")
...
system.time({
  matrix <- foreach(j=1:nsim,  .combine = 'comb', .multicombine = TRUE) %dopar% {
    ...
    for (i in seq)   {
      ...
      # Correct Rules
      rule.sel.double_bart <- length(which(rule.sel.double_bart=="xx1< 0.5" |
                                             rule.sel.double_bart=="xx1>=0.5"))
      correct.rules.bcf.itt <- length(which(rule.sel.bcf.itt=="xx1< 0.5" |
                                             rule.sel.bcf.itt=="xx1>=0.5"))
      correct.rules.ctree <- length(which(rule.sel.ctree=="xx1< 0.5" |
...
colnames(results_sims) <- c("correct_rules hct-iv",
                            "correct rules bcf-iv", 
                            "correct rules double bart")
write.csv(results_sims, file = "sim_partially_weak_scenario2_1000.csv")